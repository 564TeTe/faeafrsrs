...
function home(){
 ...
 <div class="callout"><b>${state.beginner?'零基础模式 · 第一次学，先看这里':'直接做题模式'}</b><p>${state.beginner?'第一次进入每个模块，先读必要概念，再看固定步骤和老师原题。先理解再动笔，不需要先刷选择题。':'已会基本概念，可以直接按模板做原题；不熟悉的词仍可在每章第一段展开查看。'}</p>${beginnerSwitch()}</div>
 ...
 <p class="fine score-disclaimer">74 / 26 是这份试卷的分值分组，不是保证得分。预计得分按你勾选“原题独立做对”估算；变式也独立做对后才标记模块通过。</p>
 ...
 <section id="plan48"><div class="section-head"><h2>默认计划：48 小时补考冲刺</h2><small>第一天 5.5 小时 · 第二天 6 小时净学习</small></div><div class="two-day-plan">${sprintDays.map((day,i)=>`<section class="plan-panel"><h3>${day.title}</h3><ol class="schedule">${day.tasks.map(([title,mins,target],j)=>`<li><a href="#${state.beginner&&i===0&&j===1?'ch4/beginner':target}"><span>${title}</span><b>${mins} 分钟</b></a>${i===1&&j===2?`<div class="core-gate ${corePassed()?'ready':''}"><strong>74 分检查点</strong><p>${corePassed()?'三项核心已自评通过，接下来进入 SLR。':'原题 1—5 或对应变式还有不会的，先补漏；必要时挪用后面 SLR 的时间。'}</p></div>`:''}</li>`).join('')}</ol></section>`).join('')}</div><p class="fine">把休息和睡眠留在学习时段之外。最后 60 分钟是快速模拟，不代表老师规定考试时长；先按顺序写出各题关键过程，再查漏。变式每类只有一道，原题做对后立即拿它验证。</p></section>
 <div class="study-method"><div class="method"><b>${state.beginner?'① 先理解词，再看步骤':'① 模板看 2 分钟'}</b><p>${state.beginner?'先看每章第一段的小例子，再接固定模板和原题示范。':'知道先写什么，立即回到题目。'}</p></div><div class="method"><b>② 遮住答案亲手写</b><p>不只写结论，图、集合、表都写完整。</p></div><div class="method"><b>③ 变式验证后打勾</b><p>看过答案不算通过；不会的步骤才回看。</p></div></div>`;
}
function lesson(c){
 const flow=c.pipeline.map(([label,meaning,rule,example,trap,target])=>`<tr><th scope="row">${label}<small>${meaning}</small></th><td>${rule}</td><td>${example}<div class="step-error">易错：${trap}</div><a class="mini-link" href="#exam/${target}">立即做原题 ${target.slice(1)} →</a></td></tr>`).join('');
 return heading(`${c.points} POINTS / ${c.priority}`,c.title,`本模块建议 ${c.time} 分钟；${state.beginner?'先理解必要概念，再照步骤写完整过程。':'重点在完整过程。'}`)+`
 <p>${beginnerSwitch()}</p>
 <div class="chapter-brief">${[['这章值多少分',c.points+' 分'],['考试会怎么出',c.examType],['必须掌握',c.must],['可以不学',c.skip]].map(([a,b])=>`<div><b>${a}</b><p>${b}</p></div>`).join('')}</div>
 ...
 </article><aside class="toc"><h3>按这六段顺着学</h3>${[['beginner','零基础：先理解几个词'],['quick','30 秒核心思想'],['pipeline','做题固定步骤'],['worked','老师原题示范'],['do','遮答案自己做'],['partial','最低步骤分写法']].map(([id,s])=>`<a href="#${c.id}/${id}">${s}</a>`).join('')}<div class="tip"><a href="#templates">考前 10 分钟：答题模板 →</a><br><a href="#quiz/${c.id}">概念卡住时再检查 →</a></div></aside></div>`;
}
function paperCard(e,isVariant){return `<article class="exam-card" id="${e.id}"><div class="exam-head"><div class="eyebrow">${isVariant?'同型变式 · 非试卷原题':'原题 '+e.id.slice(1)+' / '+e.points+' 分'}</div><h2>${e.title}</h2>${e.prompt}<div class="paper-links"><a href="#${e.chapter}" class="mini-link">只看必要讲解 →</a><a href="#templates/${e.templateId}" class="mini-link">查看答题模板 →</a><a href="#${isVariant?'exam/'+e.originalId:'variants/'+e.id.replace('e','v')}" class="mini-link">${isVariant?'对应原题':'做同型变式'} →</a></div></div><details><summary>写完后再展开解析</summary><div class="solution">${e.answer}<div class="self-check"><p>确认标准：没看答案，完整写出题目要求的步骤；对照发现的错误已独立重做改正。</p><button class="primary-button ${state.paper[e.id]?'done':''}" data-paper="${e.id}">${state.paper[e.id]?'✓ 已自评独立做对 · 点击撤销':'我已独立做对，记录通过'}</button><small>这是手动自评，不是自动阅卷。</small></div></div></details></article>`}
